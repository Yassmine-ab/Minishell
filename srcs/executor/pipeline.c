...
static void	add_pid(t_pids *pids, pid_t pid, t_minishell *data)
{
	...
	{
		pids->capacity *= 2;
		pids->pids = gc_realloc(pids->pids,
				sizeof(pid_t) * pids->capacity, &data->gc);
	}
		...
		if (pid == 0)
		{
			...
			if (prev_fd != -1)
			{
				if (dup2(prev_fd, STDIN_FILENO) == -1)
					error("Failed to redirect input from pipe", 1, data);
				safe_close(&prev_fd);
			}
			if (dup2(pipe_fd[WRITE_END], STDOUT_FILENO) == -1)
				error("Failed to redirect output to pipe", 1, data);
